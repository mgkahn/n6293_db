...
firebirdSetup() {
  ...
  mkdir -p "${VOLUME}/data"
  mkdir -p "${VOLUME}/restore"
  mkdir -p "${VOLUME}/etc"

## BEGIN MGK
  cp -R -u /tmp/databases/* "${VOLUME}/data/"
  cp -R -u /tmp/restore/* "${VOLUME}/restore/"
  cp -R -u /tmp/etc/* "${VOLUME}/etc/"
  cp -R -u /tmp/etc/* "${PREFIX}/skel/etc/"
##rm -fr /tmp/data /tmp/restore /tmp/etc
## END MGK
